  ...
  randomize_primary_(false)
{
  ...
  lambda_ = particleTable->FindParticle("lambda");
  
  // default particle kinematics
  particlegun_->SetParticlePosition(G4ThreeVector(0.,0.,0.*mm));
  particlegun_->SetParticleDefinition(lambda_);
...
{
  G4ParticleDefinition* particle = lambda_;  
  particlegun_->SetParticleDefinition(lambda_);
...
{
  // Define /lambda_pol/generator command directory using generic messenger class
  messenger_ 
    = new G4GenericMessenger(this, 
        "/lambda_pol/generator/", 
